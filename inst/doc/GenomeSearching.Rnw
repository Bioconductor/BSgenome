...
BSgenome data packages currently available in your version of
Bioconductor (you need to be connected to the internet for this
so \Rfunction{available.genomes} can query the Bioconductor website).
...
In this section we show how to find (or count) the occurences of some
...
which are respectively the WS100, WS120 and WS170 releases.
...
has 3 masks on it: the mask of assembly gaps, the mask of repeat
...
activate the first mask (assembly gaps), do:
<<f2>>=
...
always be \Robject{TRUE}, whatever masks are put on it:
<<f8>>=
...
Finally, this can be achieved more efficiently by simply masking the
assembly gaps:
<<f12>>=
active(masks(chrY))[1] <- TRUE
countPattern(Ebox, chrY, fixed=FALSE)
...
Note that with chromosomes that have Ns outside the assembly
gaps like chromosome 2, you need to use
\Robject{fixed=c(pattern=FALSE,subject=TRUE)} even when the mask
of assembly gaps is active:
<<f13>>=
chr2 <- Hsapiens$chr2
...
Or, alternatively, you could mask the Ns yourself with the
\Rfunction{maskMotif} function:
<<f14>>=
chr2_noNs <- maskMotif(unmasked(chr2), "N")
alphabetFrequency(chr2_noNs)
...
the usefulness of supporting masks depends on the function itself.
