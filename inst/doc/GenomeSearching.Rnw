...
With the \Rmethod{matchPattern} function, the locations of the matches are
...
The \Rmethod{mismatch} function (new in \Rpackage{Biostrings}~2)
...
Now when we use a {\it mask aware} function like
