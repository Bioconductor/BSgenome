...
It is intended for users who want to make a new \term{BSgenome data package},
not for regular users of these packages.
...
\item Some basic knowledge of the Unix/Linux command line is required. The
      ...
      text files.
\item You need access to a good Unix/Linux machine with a decent amount
...
\item You need the latest release versions of {\R} plus the
      \Rpackage{Biostrings} and \Rpackage{BSgenome} packages installed on this
      machine. To check your installation, start {\R} and try to load the
      \Rpackage{BSgenome} package.
\item Finally, you need access to the \term{source data files} of the genome
      ...
      They can be publicly available like in the case of some genome
      providers like UCSC or NCBI or you can have a private access to them.
      This document assumes that you've already downloaded them to the
      build machine or that you'll be able to do so.
\end{itemize}

Refer to the \textit{R Installation and Administration} manual
(\url{http://cran.r-project.org/doc/manuals/R-admin.html}) if you
need to install {\R} or upgrade your {\R} version, and to this page
\url{http://bioconductor.org/docs/install/} on the Bioconductor
website if you need to install or update the \Rpackage{Biostrings}
or \Rpackage{BSgenome} packages.

Questions, comments or bug reports about this document or the BSgenomeForge
functions are welcome. Please address them to the author
...
Bioconductor website and subscribe to this list if you've not already
done so.


\section{Download the source data files}

As mentioned earlier, there are 2 groups of source files: (1) the files
containing the sequence data and (2) the files containing the mask data.

Group 1 must be FASTA files. You need 1 file per sequence that you
want to put in the \term{target package}.

What you need from group 2 depends on what built-in masks you want to put
...
\item the mask of intra-contig ambiguities, aka the AMB masks;
...
file per chromosome or a single file containing the assembly gaps information
for all the chromosomes together.

You don't need any file for the AMB masks.

For the RM masks, you need one RepeatMasker \code{.out} file per chromosome.
Note that some genome providers like UCSC sometimes provide a single
\code{.out} file for all the chromosomes. BSgenomeForge doesn't support this
yet but you can work around it by splitting the big file in 1 file per
chromosome for now (alternatively, you can request support for this on The
Bioconductor Project Mailing List).

For the TRF masks, you need one Tandem Repeats Finder \code{.bed} file per
chromosome.

For example, here are the files that were used for making the
\Rpackage{BSgenome.Rnorvegicus.UCSC.rn4} package:

\begin{itemize}
\item Group 1:
      \begin{itemize}
      \item Single sequences: file \code{chromFa.tar.gz} was downloaded from
            \url{http://hgdownload.cse.ucsc.edu/goldenPath/rn4/bigZips/} and
            extracted with:
            \begin{verbatim}
      ...
      \item Multiple sequences: files \code{upstream1000.fa.gz},
            ...
            downloaded from \url{http://hgdownload.cse.ucsc.edu/goldenPath/rn4/bigZips/}
...
\item Group 2:
      \begin{itemize}
      \item AGAPS masks: all the \code{chr*\_gap.txt.gz} files were downloaded
            from \url{ftp://hgdownload.cse.ucsc.edu/goldenPath/rn4/database/}.
            This was done with the standard Unix/Linux \code{ftp} command:
            \begin{verbatim}
...
and that they are located in the \code{seqs\_srcdir} folder for group 1 and in
the \code{masks\_srcdir} folder for group 2.

Note that all the \term{source data files} should be located directly in the
\code{seqs\_srcdir} or \code{masks\_srcdir} folders and not in subfolders of
these folders. For example, depending on the genome, UCSC provides either a
...
one level down the \code{seqs\_srcdir} folder (granted that you were in this
...
The format of this file is DCF (Debian Control File), like for the
\code{DESCRIPTION} file of any {\R} package. The valid fields of a
\term{seed file} are divided in 3 categories:
\begin{enumerate}
...
Alternatively, the reader in a hurry can look at the \term{seed files} included
in the \Rpackage{BSgenome} package. Those files were used to forge the packages
provided by the Bioconductor project.
<<>>=
library(BSgenome)
...
\item \code{Description}, \code{Version}, \code{Author}, \code{Maintainer},
      ...
      Please refer to section \code{1.1.1 The DESCRIPTION file} of the
      \textit{Writing R Extensions} manual
      (\url{http://cran.r-project.org/doc/manuals/R-exts.html#The-DESCRIPTION-file})
      for more information about these fields.
      If you plan to distribute the package that you are going to make, please
...
\item \code{species}: The name of the species.
      ...
      this field corresponds to the \code{SPECIES} column of this table:
...
\item \code{provider\_version}: The provider-side version of the genome.
      ...
      For the packages built by the Bioconductor project from a UCSC genome,
      this field corresponds to the \code{UCSC VERSION} field of this table:
      \url{http://genome.ucsc.edu/FAQ/FAQreleases#release1}.

\item \code{release\_date}: When this assembly of the genome was released.
      For the packages built by the Bioconductor project from a UCSC genome,
      this field corresponds to the \code{RELEASE DATE} field of this table:
      \url{http://genome.ucsc.edu/FAQ/FAQreleases#release1}.

\item \code{release\_name}: The release name or build number of this assembly
      of the genome.
      For the packages built by the Bioconductor project from a UCSC genome,
      this field corresponds to the \code{RELEASE NAME} field of this table:
      \url{http://genome.ucsc.edu/FAQ/FAQreleases#release1}.
...
\item \code{organism\_biocview}: The official biocViews term for this organism.
      ...
      should be replaced by underscores. This matters only if the \term{target
      package} is going to be added to a Bioconductor repository since this
      field will determine under which of the following subviews the package
      will appear:
      \url{http://bioconductor.org/packages/release/Organism.html}.
...
\item \code{seqs\_srcdir}, \code{masks\_srcdir}: The path to the local folders
      containing the \term{source data files} for the sequences and for the
      masks, respectively. 

\item \code{seqfiles.prefix}, \code{seqfiles.suffix}: [OPTIONAL]

\item \code{AGAPSfiles.type}: [OPTIONAL]

\item \code{AGAPSfiles.name}: [OPTIONAL]

\item \code{AGAPSfiles.prefix}: [OPTIONAL]

\item \code{AGAPSfiles.suffix}: [OPTIONAL]
\end{itemize}
...
Start \R, load the \Rpackage{BSgenome} package and call the
\Rfunction{forgeBSgenomeDataPkg} function on your \term{seed file} to forge
the \term{target package}. For example, if the path to your \term{seed file}
is \code{"path/to/my/seed"}, just do:
...
leave \R, and build the source package (tarball) with
\begin{verbatim}
    R CMD build <pkgdir>
\end{verbatim}
