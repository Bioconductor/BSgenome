...
% package that will trigger a MiKTeX-pdflatex error if \url is used like this:
%   \url{http://some/url}
% This error is hard to reproduce. It will look like:
...
% I've observed this with the following MiKTeX-pdflatex version:
%   MiKTeX-pdfTeX 2.7.3147 (1.40.9) (MiKTeX 2.7)
% A work around is to always use \url like this:
%   {\url{http://some/url}}
% Without the extra level of braces, this file will trigger the infamous error.
\usepackage{hyperref}
...
\item \code{Description}, \code{Version}, \code{Author}, \code{Maintainer},
      ...
      Please refer to the \textit{The DESCRIPTION file} section of the
