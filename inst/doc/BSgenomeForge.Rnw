...
\subsection{Sequence data (group1)}
...
Here the name of each file must be of the form \textit{<seqname>}\code{.fa.out}
or \textit{<seqname>}\code{.out}.
...
chromosome. Here the name of each file must be of the form
\textit{<seqname>}\code{.bed}.
...
\item Group 2:
      ...
      \item AGAPS masks: all the \code{chr*\_gap.txt.gz} files (UCSC ``gap''
            ...
            \url{ftp://hgdownload.cse.ucsc.edu/goldenPath/rn4/database/}.
            with the standard Unix/Linux \code{ftp} command:
            \begin{verbatim}
...
\item \code{organism\_biocview}: The official biocViews term for this organism.
      ...
      repository since it will determine under which of the following subviews
...
\item \code{seqfiles.prefix}, \code{seqfiles.suffix}: [OPTIONAL] Default value
      for \code{seqfiles.suffix} is \code{.fa}.

\item \code{AGAPSfiles.type}, \code{AGAPSfiles.name}, \code{AGAPSfiles.prefix},
      \code{AGAPSfiles.suffix}: [OPTIONAL]
\end{itemize}
