...
(\code{bioconductor@r-project.org}). Don't forget to visit the
...
In this document, we call \term{target packages} the \term{BSgenome
data packages} that we're going to forge (one target package with
bare sequences and a 2nd target package with masked sequences).
...
The sequence data must be in FASTA files. You need 1 file per sequence that
you want to put in the \term{target package}. The name of each file must be
of the form \textit{<prefix>}\textit{<seqname>}\textit{<suffix>} where
...
Here is how the \term{sequence data files} for the
...
good idea), extracted, and eventually renamed all the \term{sequence data
files}, and that they are located in the \textit{<seqs\_srcdir>} folder.
...
\item \code{Title}: The title of the package. E.g. \code{Full genome sequences
      for Rattus norvegicus (UCSC version rn4)}.
...
\item \code{Description}, \code{Version}, \code{Author}, \code{Maintainer},
      ...
      license of the \term{sequence data files} if any.
\end{itemize}
...
\item \code{source\_url}: The permanent URL where the \term{sequence data
      files} used to forge this package can be found.
...
\item \code{SrcDataFiles1}, \code{SrcDataFiles2}: [OPTIONAL] Some arbitrary
      ...
      where the \term{sequence data files} are coming from.
      \code{SrcDataFiles2} can contain any extra information and/or URLs.
      Permanent URLs are a must.
...
\item \code{seqs\_srcdir}: The path to the \textit{<seqs\_srcdir>} folder.
...
The \code{mode} argument was added in Bioconductor 2.14 and can be either
\code{"rda"}, \code{"fa"}, or \code{"fa.rz"}. See \code{?forgeBSgenomeDataPkg}
for the details.
%Starting with Bioconductor 2.14, \code{"fa.rz"} is the recommended mode.
...
From now we assume that you've downloaded (checking the md5sums is always a
good idea) and eventually extracted all the \term{mask data files},
...
\begin{itemize}
\item \code{organism}, \code{species}, \code{provider},
      \code{provider\_version}, \code{release\_date}, \code{release\_name},
      \code{organism\_biocview}:
      Should be the same as for \term{reference target package}.
...
\item \code{SrcDataFiles1}, \code{SrcDataFiles2}: [OPTIONAL] Some arbitrary
      text that will be copied to the \code{Note} section of the man pages of
      the \term{target package}. \code{SrcDataFiles1} should describe briefly
      where the \term{mask data files} are coming from.
      \code{SrcDataFiles2} can contain any extra information and/or URLs.
      Permanent URLs are a must.
...
library(BSgenome)
seed_files <- system.file("extdata", "GentlemanLab", package="BSgenome")
list.files(seed_files, pattern="\\.masked-seed$")
rn4_masked_seed <- list.files(seed_files, pattern="rn4\\.masked", full.names=TRUE)
