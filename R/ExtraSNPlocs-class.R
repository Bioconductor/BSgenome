...
newExtraSNPlocs <- function(provider, provider_version,
                            ...
                            data_pkgname, data_dirpath)
{
    data_serialized_objnames <- c(
        "ExtraSNPcount",
        paste(seqlevels(reference_genome), "_extrasnplocs", sep="")
    )
    new("ExtraSNPlocs",
        provider=provider,
...
### WARNING: Improper calls to .get_ExtraSNPlocs_data() by the
### .create_object() function can lead to infinite recursive loops!
.create_object <- function(objname)
{
    if (objname == "empty_extrasnplocs") {
...
.get_ExtraSNPlocs_data <- function(x, objname, caching=TRUE)
{
...
setMethod("snpcount", "ExtraSNPlocs",
    function(x)
    {
        objname <- "ExtraSNPcount"
        ans <- .get_ExtraSNPlocs_data(x, objname)
...
### Load raw extra snplocs.
.load_raw_extrasnplocs <- function(x, seqname, caching)
{
    objname <- paste(seqname, "_extrasnplocs", sep="")
    ans <- .get_ExtraSNPlocs_data(x, objname, caching=caching)
    empty_extrasnplocs <- .get_ExtraSNPlocs_data(x, "empty_extrasnplocs")
...
{
    all_rsids <- .get_ExtraSNPlocs_data(x, "all_rsids")
...
### Get user-friendly extra snplocs.
.get_ufextrasnplocs <- function(x, seqname, caching)
{
    rsids <- as.character(.load_rsids(x, seqname))
    extrasnplocs <- .load_raw_extrasnplocs(x, seqname, caching)
    alleles <- safeExplode(rawToChar(extrasnplocs$alleles))
    data.frame(RefSNP_id=rsids,
               alleles_as_ambig=alleles,
               loc=extrasnplocs$loc,
               stringsAsFactors=FALSE)
}

.ExtraSNPlocsAsGranges <- function(x, ufextrasnplocs, seqname)
{
    if (is(seqname, "Rle")) {
        if (length(seqname) != nrow(ufextrasnplocs)
         || !identical(levels(seqname), seqlevels(x)))
            stop("when an Rle, 'seqname' must be a factor Rle ",
                 "of length 'nrow(ufextrasnplocs)' and levels 'SEQNAMES'")
        ans_seqnames <- seqname
    } else {
        if (!is.factor(seqname))
            seqname <- factor(seqname, levels=seqlevels(x))
        if (length(seqname) == 1L)
            ans_seqnames <- Rle(seqname, nrow(ufextrasnplocs))
        else if (length(seqname) == nrow(ufextrasnplocs))
            ans_seqnames <- Rle(seqname)
        else
            stop("'length(seqname)' must be 1 or 'nrow(ufextrasnplocs)'")
    }
    if (nrow(ufextrasnplocs) == 0L)
        ans_ranges <- IRanges()
    else
        ans_ranges <- IRanges(start=ufextrasnplocs$loc, width=1L)
    ans_strand <- Rle(strand("+"), nrow(ufextrasnplocs))
    ans <- GRanges(seqnames=ans_seqnames,
                   ranges=ans_ranges,
                   strand=ans_strand,
                   RefSNP_id=ufextrasnplocs$RefSNP_id,
                   alleles_as_ambig=ufextrasnplocs$alleles_as_ambig)
...
setMethod("snplocs", "ExtraSNPlocs",
    function(x, seqname, as.GRanges=FALSE, caching=TRUE)
    {
        ...
        if (!as.GRanges) {
            ...
            return(.get_ufextrasnplocs(x, seqname, caching))
        }
        if (length(seqname) == 0L) {
            empty_ufextrasnplocs <- .get_ExtraSNPlocs_data(x, "empty_ufextrasnplocs")
            ans <- .ExtraSNPlocsAsGranges(x, empty_ufextrasnplocs, character(0))
            return(ans)
        }
        list_of_ufextrasnplocs <- lapply(seqname,
                                    .get_ufextrasnplocs, x=x, caching=caching)
        ufextrasnplocs <- do.call(rbind, list_of_ufextrasnplocs)
        seqnames <- Rle(factor(seqname, levels=seqlevels(x)),
                        unlist(lapply(list_of_ufextrasnplocs, nrow), use.names=FALSE))
        .ExtraSNPlocsAsGranges(x, ufextrasnplocs, seqnames)
    }
)
...
### the SNP id and the value is the row index in the serialized extrasnplocs
### data frame corresponding to the SNP id.
        ...
        if (length(rowidx) == 0L) {
            ...
            loc_list <- lapply(names(rowidx_list),
                function(seqname) {
                    ...
                    .load_raw_extrasnplocs(x, seqname, caching)$loc[idx]
                })
    ...
    function(x, snpid, caching=TRUE)
    {
        ...
        if (length(rowidx) == 0L) {
            ...
            alleles_list <- lapply(names(rowidx_list),
                function(seqname) {
                    idx <- rowidx_list[[seqname]]
                    .load_raw_extrasnplocs(x, seqname, caching)$alleles[idx]
                })
...
setMethod("snpid2grange", "ExtraSNPlocs",
    function(x, snpid, caching=TRUE)
    {
        ...
        ufextrasnplocs <- data.frame(RefSNP_id=as.character(snpid),
                                alleles_as_ambig=unname(alleles),
                                loc=unname(loc),
                                stringsAsFactors=FALSE)
        .ExtraSNPlocsAsGranges(x, ufextrasnplocs, names(loc))
    }
)
